...
async function loginAndOnboard(page: import("playwright").Page) {
  ...
  const email = `coach_shot_${stamp}@childcompass.test`;
  ...
  await page.waitForURL(/\/(verify-email|onboarding|today|dashboard|coach)/, { timeout: 30000 });
...
async function main() {
  ...
  await page.goto("http://localhost:3000/coach", { waitUntil: "networkidle" });
  await page.waitForSelector("text=/Talk with Child Compass/i", { timeout: 30000 });
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.waitForTimeout(1000);

  await page.screenshot({
    path: path.join(outDir, "coach-after-hero.png"),
    fullPage: false,
  });

  await page.setViewportSize({ width: 1280, height: 2200 });
  await page.screenshot({
    path: path.join(outDir, "coach-after-full.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(outDir, "coach-after-mobile.png"),
    fullPage: true,
  });

  console.log("Coach screenshots saved to screenshots/");
  await browser.close();
}
