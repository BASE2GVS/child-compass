...
async function loginAndOnboard(page: import("playwright").Page) {
  ...
  const email = `track_shot_${stamp}@childcompass.test`;
  ...
  await page.waitForURL(/\/(verify-email|onboarding|today|track)/, { timeout: 30000 });
...
async function main() {
  ...
  await page.waitForSelector("text=/Your Family Journey/i", { timeout: 30000 });
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.waitForTimeout(800);

  await page.screenshot({ path: path.join(outDir, "track-after-hero.png"), fullPage: false });

  await page.setViewportSize({ width: 1280, height: 2800 });
  await page.screenshot({ path: path.join(outDir, "track-after-full.png"), fullPage: true });

  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto("http://localhost:3000/track", { waitUntil: "domcontentloaded" });
  await page.waitForTimeout(800);
  await page.screenshot({ path: path.join(outDir, "track-after-mobile.png"), fullPage: true });

  console.log("Track screenshots saved to screenshots/");
  await browser.close();
}
