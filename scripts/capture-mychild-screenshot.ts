...
async function loginAndOnboard(page: import("playwright").Page) {
  ...
  const email = `mychild_shot_${stamp}@childcompass.test`;
  ...
  await page.waitForURL(/\/(verify-email|onboarding|today|dashboard|compass)/, { timeout: 30000 });
...
async function main() {
  ...
  await page.goto("http://localhost:3000/compass", { waitUntil: "networkidle" });
  await page.waitForSelector("text=/learning a little more/i", { timeout: 30000 });
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.waitForTimeout(800);

  await page.screenshot({ path: path.join(outDir, "mychild-after-hero.png"), fullPage: false });

  await page.setViewportSize({ width: 1280, height: 3200 });
  await page.screenshot({ path: path.join(outDir, "mychild-after-full.png"), fullPage: true });

  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto("http://localhost:3000/compass", { waitUntil: "networkidle" });
  await page.waitForTimeout(800);
  await page.screenshot({ path: path.join(outDir, "mychild-after-mobile.png"), fullPage: true });

  console.log("My Child screenshots saved to screenshots/");
  await browser.close();
}
