...
async function loginAndOnboard(page: import("playwright").Page) {
  ...
  const email = `checkin_shot_${stamp}@childcompass.test`;
...
async function main() {
  ...
  await page.goto("http://localhost:3000/check-in", { waitUntil: "networkidle" });
  await page.waitForSelector("text=/How has today been/i", { timeout: 30000 });
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.waitForTimeout(800);

  await page.screenshot({
    path: path.join(outDir, "checkin-after-hero.png"),
    fullPage: false,
  });

  await page.getByRole("radio", { name: /Okay/i }).first().click();
  await page.waitForTimeout(600);
  await page.screenshot({
    path: path.join(outDir, "checkin-after-question.png"),
    fullPage: false,
  });
  ...
  await page.waitForTimeout(800);
  await page.screenshot({
    path: path.join(outDir, "checkin-after-full.png"),
    fullPage: true,
  });
  ...
  await page.waitForTimeout(800);
  await page.screenshot({
    path: path.join(outDir, "checkin-after-mobile.png"),
    fullPage: true,
  });

  console.log("Check-in screenshots saved to screenshots/");
  await browser.close();
}
