...
export default async function MonthlyStoryDraftPage({
  params,
  searchParams,
}: {
  ...
  const card = overview.recentMonths.find((month) => month.id === periodId);
  if (!card) redirect(`/journey?child=${context.child.id}`);
