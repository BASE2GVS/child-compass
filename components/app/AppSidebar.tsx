...
export default function AppSidebar({
  ...
  return (
    <aside className="cc-shell-sidebar relative z-[2] hidden w-[17.5rem] shrink-0 flex-col lg:flex">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-2.5 pb-4" aria-label="Main navigation">
          ...
          return (
            <Link
              ...
              className={`relative flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-[0.9375rem] font-medium transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--cc-teal)]/40 ${
                active
                  ? "bg-gradient-to-r from-[var(--cc-teal)]/12 to-[var(--cc-teal)]/6 text-[var(--cc-teal-deep)] shadow-[inset_2px_0_0_0_var(--cc-teal)]"
                  : isAsk
                    ? "text-[var(--cc-ink)] hover:bg-white/50"
                    : "text-[var(--cc-ink-muted)] hover:bg-white/40 hover:text-[var(--cc-ink)]"
              }`}
            >
              <NavIcon d={item.icon} />
